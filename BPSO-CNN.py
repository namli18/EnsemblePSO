# BPSO with CNN (Feature selection and Classification)
# last update date: 18.08.2022
...
class CNN_model():
    def fit (self, X_train, y_train):
        ...
        conv1 = Conv1D(20, round(X_train.shape[1]/2), activation="sigmoid")(input)
        batch1 = BatchNormalization()(conv1)
        conv2 = Conv1D(4, round(X_train.shape[1]/2)-2, activation="sigmoid")(batch1)
...
def f_per_particle(b, alpha):
    ...
    j = (alpha * (1.0 - P) + (1.0 - alpha) * (1 - (X_subset.shape[1] / dimensions)))
    return j
